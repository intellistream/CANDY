...
class VectorDBStream {
 public:
  ...
  static VectorDBStream from(const std::vector<std::vector<float>>& input_vectors);

  // Map transformation: Applies a function to each vector
  VectorDBStream& map(const std::function<std::vector<float>(const std::vector<float>&)>& func);

  // Filter transformation: Filters vectors based on a predicate
  VectorDBStream& filter(const std::function<bool(const std::vector<float>&)>& predicate);

  // LLM embedding generation: Convert input text to embeddings
  VectorDBStream& map_to_embedding(const std::function<std::vector<float>(const std::string&)>& embedding_func);
  ...
  std::vector<std::vector<float>> query_nearest(const std::vector<float>& query_vec, size_t k);
