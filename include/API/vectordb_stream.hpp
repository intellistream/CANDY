...
public:
 ...
 VectorDBStream& map(const std::function<torch::Tensor(const torch::Tensor&)>& func);

 // Filter transformation: Filters tensors based on a predicate
 VectorDBStream& filter(const std::function<bool(const torch::Tensor&)>& predicate);

 // LLM embedding generation: Convert input text to embeddings
 VectorDBStream& map_to_embedding(const std::function<torch::Tensor(const std::string&)>& embedding_func);
 ...
 std::vector<torch::Tensor> query_nearest(const torch::Tensor& query_tensor, size_t k);
