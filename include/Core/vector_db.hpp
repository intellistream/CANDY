...
class VectorDB {
 public:
  ...
  VectorDB(size_t dimensions,
           std::shared_ptr<SearchAlgorithm> search_algorithm = nullptr);
  ~VectorDB();
  ...
  std::vector<std::vector<float>> query_nearest_vectors(
  ...
  void insert_streaming_vector(
      const std::vector<float>& vec);  // Insert vector into the stream buffer
