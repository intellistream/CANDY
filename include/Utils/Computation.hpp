...
inline torch::Tensor amm_crs(torch::Tensor& a, torch::Tensor& b, int64_t ss) {
  TORCH_CHECK(a.dim() == 2 && b.dim() == 2,
              "amm_crs:Both a and b must be 2D tensors.");
  TORCH_CHECK(a.size(1) == b.size(0),
              "amm_crs:Shape mismatch: a.size(1) must equal b.size(0).");

  int64_t n = a.size(1);
  ss = std::min(ss, n);  // Ensure ss is within bounds
  // Probability distribution
  torch::Tensor probs = torch::ones(n) / n;  // default: uniform
  // torch::Tensor weights = torch::abs(b).sum(1); // 以特征值的绝对值为权重
  // torch::Tensor probs = weights / weights.sum(); // 归一化为概率
  auto crsIndices = torch::multinomial(probs, ss, false);  // 按权重采样
...
inline torch::Tensor amm_smppca(torch::Tensor& A, torch::Tensor& B,
                                int64_t sketchsize) {
  // Step 1: Input A:n1*d B:d*n2
  A = A.t();  // d*n1
  TORCH_CHECK(A.size(0) == B.size(0),
              "amm_smppca:Shape mismatch: A and B must have the same feature "
              "dimension.");
  ...
  torch::Tensor pi =
      1 / std::sqrt(k) * torch::randn({k, d});   // Gaussian sketching matrix
  ...
  torch::Tensor col_norm_A = torch::linalg::vector_norm(
      A, 2, {0}, false, c10::nullopt);  // ||Ai|| for i in [n1]
  torch::Tensor col_norm_B = torch::linalg::vector_norm(
      B, 2, {0}, false, c10::nullopt);  // ||Bj|| for j in [n2]

  // 3.2 column norms of A_tilde and B_tilde
  torch::Tensor col_norm_A_tilde =
      torch::linalg::vector_norm(A_tilde, 2, {0}, false,
                                 c10::nullopt);  // ||Ai|| for i in [n1]
  torch::Tensor col_norm_B_tilde =
      torch::linalg::vector_norm(B_tilde, 2, {0}, false,
                                 c10::nullopt);  // ||Bj|| for j in [n2]

  // Step 4: Compute M_tilde
  torch::Tensor col_norm_A_col_norm_B =
      torch::matmul(col_norm_A.reshape({n1, 1}), col_norm_B.reshape({1, n2}));

  torch::Tensor col_norm_A_tilde_col_norm_B_tilde = torch::matmul(
      col_norm_A_tilde.reshape({n1, 1}), col_norm_B_tilde.reshape({1, n2}));
  torch::Tensor mask = (col_norm_A_tilde_col_norm_B_tilde == 0);
  col_norm_A_tilde_col_norm_B_tilde.masked_fill_(
      mask, 1e-6);  // incase divide by 0 in next step

  torch::Tensor ratio =
      torch::div(col_norm_A_col_norm_B, col_norm_A_tilde_col_norm_B_tilde);
...
inline torch::Tensor AMM_Compute_DotproductSimilarity(torch::Tensor& a,
                                                      torch::Tensor& b,
                                                      int64_t ss,
                                                      AMMTYPE ammtype) {
  TORCH_CHECK(a.dim() == 2 && b.dim() == 2,
              "AMM_Compute:Inputs a and b must be 2D tensors.");
  TORCH_CHECK(a.size(1) == b.size(0),
              "AMM_Compute:Shape mismatch: a.size(1) must equal b.size(0).");

  switch (ammtype) {
    case NONE_AMM:
      return torch::matmul(a, b);
    case AMM_CRS:
      return amm_crs(a, b, ss);
    case AMM_SMPPCA:
      return amm_smppca(a, b, ss);
  }
  return torch::matmul(a, b);
}

// From dot product similarity to L2 distance
inline torch::Tensor pairwise_euclidean_distance(torch::Tensor A,
                                                 torch::Tensor B,
                                                 AMMTYPE ammtype,
                                                 int64_t sketchsize) {
  ...
  if (!ammtype) {
    ...
    // AB_dot = torch::zeros({A.size(0), B.size(1)});
  ...
  torch::Tensor D_squared = B_norm - 2 * AB_dot;  // shape: n x m
...
inline torch::Tensor pairwise_euclidean_distance(torch::Tensor A,
  ...
  // Step 4: return L2 distance 2D-tensor
  return D_squared;
}
}  // namespace CANDY
#endif  // COMPUTATION_H
