...
class TensorOP {
 public:
  ...
  static bool insertRows(torch::Tensor* tHead, torch::Tensor* tTail,
                         int64_t startRow);
  static bool insertRows(TensorPtr tHead, TensorPtr tTail, int64_t startRow);
  static bool editRows(torch::Tensor* tHead, const torch::Tensor* tTail,
                       int64_t startRow);
  static bool editRows(TensorPtr tHead, TensorPtr tTail, int64_t startRow);
  static bool deleteRowBufferMode(torch::Tensor* tensor, int64_t rowIdx,
                                  int64_t* lastNNZ);
  static bool deleteRowBufferMode(TensorPtr tensor, int64_t rowIdx,
                                  int64_t* lastNNZ);
  static bool deleteRowsBufferMode(torch::Tensor* tensor,
                                   std::vector<int64_t>& rowIdx,
                                   int64_t* lastNNZ);
  static bool deleteRowsBufferMode(TensorPtr tensor,
                                   std::vector<int64_t>& rowIdx,
                                   int64_t* lastNNZ);
  static bool appendRowsBufferMode(torch::Tensor* tHead,
                                   const torch::Tensor* tTail, int64_t* lastNNZ,
                                   int64_t customExpandSize = 0);
  static bool appendRowsBufferMode(TensorPtr tHead, TensorPtr tTail,
                                   int64_t* lastNNZ,
                                   int64_t customExpandSize = 0);
  ...
  static bool tensorFromFlatBin(torch::Tensor* tensor,
