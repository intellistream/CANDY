...
class ConfigParser {
 public:
  ...
  // Construct ConfigParser and parse given filename. See ini.h for more info
  // about the parsing.
  ConfigParser(std::string filename);

  // Construct ConfigParser and parse given file. See ini.h for more info
  // about the parsing.
...
/**
 * @brief Construct an ConfigParser object from a file name
 * @param filename The name of the INI file to parse
 * @throws std::runtime_error if there is an error parsing the INI file
 */
inline ConfigParser::ConfigParser(std::string filename) {
  _error = ini_parse(filename.c_str(), ValueHandler, this);
  ParseError();
}

/**
 * @brief Construct an ConfigParser object from a file pointer
 * @param file A pointer to the INI file to parse
 * @throws std::runtime_error if there is an error parsing the INI file
 */
inline ConfigParser::ConfigParser(FILE* file) {
  _error = ini_parse_file(file, ValueHandler, this);
  ParseError();
}

inline int ConfigParser::ParseError() const {
  switch (_error) {
    case 0:
      break;
    case -1:
 ...
 * @return The list of sections found in ini file
 */
inline const std::set<std::string> ConfigParser::Sections() const {
 ...
 * @return The list of keys in the given section
 */
inline const std::set<std::string> ConfigParser::Keys(std::string section) const {
 ...
 */
inline const std::unordered_map<std::string, std::string> ConfigParser::Get(
    std::string section) const {
 ...
 */
template <typename T>
inline T ConfigParser::Get(const std::string& section,
                        const std::string& name) const {
  auto const _section = Get(section);
 ...
 */
template <typename T>
inline T ConfigParser::Get(const std::string& section, const std::string& name,
                        T&& default_v) const {
  try {
 ...
 */
template <typename T>
inline std::vector<T> ConfigParser::GetVector(const std::string& section,
                                           const std::string& name) const {
 ...
 * if not found
 *
 * @see ConfigParser::GetVector
 */
template <typename T>
inline std::vector<T> ConfigParser::GetVector(
 ...
 */
template <typename T>
inline void ConfigParser::InsertEntry(const std::string& section,
 ...
 */
template <typename T>
inline void ConfigParser::UpdateEntry(const std::string& section,
                                   const std::string& name, const T& v) {
...
template <typename T>
inline std::string ConfigParser::V2String(const T& v) const {
  std::stringstream ss;
  ss << v;
  return ss.str();
}
...
template <typename T>
inline std::string ConfigParser::Vec2String(const std::vector<T>& v) const {
  if (v.empty()) {
    return "";
  }
...
template <typename T>
inline T ConfigParser::Converter(const std::string& s) const {
  try {
    T v{};
...
inline const bool ConfigParser::BoolConverter(std::string s) const {
...
inline int ConfigParser::ValueHandler(void* user, const char* section,
