...
/**
 * @class AbstractIndex
 * @brief The abstract base class for an ANNS index approach
 */
class AbstractIndex {
protected:
...
public:
  ...
  /**
   * @brief Load the initial tensors and their corresponding uint64_t objects
   * @param t The tensor to load
   * @param u64s The list of corresponding uint64_t values
   * @return bool Whether the loading is successful
   */
  virtual bool loadInitialU64Object(const torch::Tensor& t, const std::vector<uint64_t>& u64s) = 0;
  ...
  /**
   * @brief Insert a tensor and its corresponding uint64_t objects
   * @param t The tensor to insert
   * @param u64s The list of corresponding uint64_t values
   * @return bool Whether the insertion is successful
   */
  virtual bool insertU64Object(const torch::Tensor& t, const std::vector<uint64_t>& u64s) = 0;
 ...
 */
typedef std::shared_ptr<AbstractIndex> AbstractIndexPtr;
