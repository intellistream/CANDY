...
private:
    ...
    VertexPtr greedy_update_nearest(const std::vector<float> &query_vec, int level, VertexPtr nearest,
    ...
    void search_layer(const std::vector<float> &query_vec, size_t k,
                      std::priority_queue<std::pair<float, VertexPtr> > &candidates,
                      std::priority_queue<std::pair<float, VertexPtr> > &top_candidates) const {
