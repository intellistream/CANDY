...
class SeparateKNNSearch : public SeparateANNSBase {
 ...
 public:
  ...
  std::vector<torch::Tensor> searchTensor(const torch::Tensor& t,
                                          int64_t k) override;
  std::vector<torch::Tensor> deleteTensor(const torch::Tensor& t,
                                          int64_t k) override;
  bool reviseTensor(const torch::Tensor& t, const torch::Tensor& w) override;
  std::vector<torch::Tensor> findKnnTensorBurst(const torch::Tensor& q,
                                                int64_t k);
