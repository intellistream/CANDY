...
class Vamana final : public ANNSBase {
 ...
 protected:
  ...
  bool deleteBatch();
  void greedy_search(idx_t s, const torch::Tensor& q,
                     priority_of_distAndId_Less& results_, int64_t k);
  void robust_prune(idx_t p, unordered_map<idx_t, float>& V, float alpha,
                    int64_t R);
  void add_links_starting_from(idx_t start_id, idx_t nearest_id,
                               float d_nearest);
  void shrink_neighbor_list(priority_of_distAndId_Less& resultSet1, int64_t R);
  void shrink_neighbor_list_robust(priority_of_distAndId_Greater& input,
  ...
  priority_of_distAndId_Less search_base_layerST(idx_t ep,
                                                 ...
                                                 long ef);
  void greedy_update_nearest(idx_t nearest, float& d_nearest,
                             const torch::Tensor& q);
};
}  // namespace CANDY_ALGO
#endif  //VAMANA_HPP
