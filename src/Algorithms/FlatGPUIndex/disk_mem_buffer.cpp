...
//
#include <Algorithms/FlatGPUIndex/disk_mem_buffer.h>
#include <c10/util/Logging.h>

namespace CANDY_ALGO {
void PlainMemBufferTU::init(int64_t vecDim, int64_t bufferSize,
                            int64_t _tensorBegin, int64_t _u64Begin,
                            int64_t _dmaSize) {
  //diskInfo.
...
torch::Tensor PlainMemBufferTU::getTensorInline(int64_t startPos,
                                                int64_t endPos) {
  ...
  */
  torch::Tensor ru = inMemTensor.slice(0, startPos - cacheT.beginPos,
                                       endPos - cacheT.beginPos);
  return ru;
}

std::vector<uint64_t> PlainMemBufferTU::getU64(int64_t startPos,
                                               int64_t endPos) {
 ...
 */
  if (startPos >= cacheT.beginPos && endPos <= cacheT.endPos) {
    cacheT.buffer.slice(0, startPos - cacheT.beginPos,
                        endPos - cacheT.beginPos) = t;
    return true;
  } else {
...
bool PlainMemBufferTU::reviseU64(int64_t startPos,
...
bool PlainMemBufferTU::deleteU64(int64_t startPos, int64_t endPos) {
  ...
  diskOffset =
      (diskInfo.u64Cnt - delSize) * sizeof(uint64_t) + 512 + bufferBegin;
