...
std::vector<torch::Tensor> SeparateANNSBase::searchTensor(
    const torch::Tensor& t, int64_t k) {
  return storage_engine->getVectorByVids(findKnnTensor(t, k));
}

std::vector<torch::Tensor> SeparateANNSBase::deleteTensor(
    const torch::Tensor& t, int64_t k) {
  return storage_engine->deleteTensor(findKnnTensor(t, k));
}

bool SeparateANNSBase::reviseTensor(const torch::Tensor& t,
                                    const torch::Tensor& w) {
  vector<int64_t> id = findKnnTensor(t, 1);
  return storage_engine->reviseTensor(w, id[0]);
}

std::vector<int64_t> SeparateANNSBase::findKnnTensor(const torch::Tensor& t,
                                                     int64_t k) {
