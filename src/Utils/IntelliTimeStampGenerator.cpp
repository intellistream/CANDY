...
bool INTELLI::TimeStampGenerator::setConfig(INTELLI::ConfigMapPtr cfg) {
  cfgGlobal = cfg;
  ...
  timeStamper_zipfEventFactor =
      cfg->tryDouble("timeStamper_zipfEventFactor", 0.1, true);
...
void INTELLI::TimeStampGenerator::generateEvent() {
  uint64_t maxTime = testSize * 1000 * 1000 / eventRateTps;
  if (staticDataSet) {
    eventS.resize(testSize);
    eventS.assign(
        testSize,
        0);  // Create vector of size 'size' with all elements set to 0
  } else if (timeStamper_zipfEvent) {
    INTELLI_INFO("Use zipf for event time, factor=" +
                 to_string(timeStamper_zipfEventFactor));
    INTELLI_INFO("maxTime=" + to_string(maxTime) + "us" + "rate=" +
                 to_string(eventRateTps) + "K, cnt=" + to_string(testSize));
    eventS = md.genZipfTimeStamp<uint64_t>(testSize, maxTime,
                                           timeStamper_zipfEventFactor);
  } else {
...
INTELLI::TimeStampGenerator::constructTimeStamps(
    std::vector<uint64_t> _eventS, std::vector<uint64_t> _arrivalS) {
  size_t len = _eventS.size();
  std::vector<INTELLI::IntelliTimeStampPtr> ru =
      std::vector<INTELLI::IntelliTimeStampPtr>(len);
...
void INTELLI::TimeStampGenerator::generateFinal() {
  myTs = constructTimeStamps(eventS, eventS);
}

std::vector<INTELLI::IntelliTimeStampPtr>
INTELLI::TimeStampGenerator::getTimeStamps() {
  return myTs;
}