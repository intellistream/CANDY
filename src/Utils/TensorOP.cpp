...
namespace INTELLI {
  ...
  bool TensorOP::deleteRows(torch::Tensor *tensor, std::vector<int64_t> &rowIdx) {
    ...
    for (int64_t row: rowIdx) {
      if (row >= tensor->size(0)) return false;
      rowMask[row] = 0;
    }
    *tensor = tensor->index({rowMask.nonzero().squeeze()});
    return true;
  ...
  bool TensorOP::insertRows(torch::Tensor *tHead, torch::Tensor *tTail, int64_t startRow) {
    if (tHead->size(1) != tTail->size(1)) return false;
    auto topPart = tHead->index({torch::indexing::Slice(torch::indexing::None, startRow)});
    auto bottomPart = tHead->index({torch::indexing::Slice(startRow, torch::indexing::None)});
  ...
  bool TensorOP::editRows(torch::Tensor *tHead, const torch::Tensor *tTail, int64_t startRow) {
    ...
    if (endRow > tHead->size(0)) {
      tHead->slice(0, startRow, tHead->size(0)) = tTail->slice(0, 0, tHead->size(0) - startRow);
    } else {
  ...
  bool TensorOP::deleteRowBufferMode(torch::Tensor *tensor, int64_t rowIdx, int64_t *lastNNZ) {
    if (rowIdx >= tensor->size(0) || *lastNNZ >= tensor->size(0) || rowIdx > *lastNNZ) return false;
    tensor->slice(0, rowIdx, rowIdx + 1) = tensor->slice(0, *lastNNZ, *lastNNZ + 1);
    tensor->slice(0, *lastNNZ, *lastNNZ + 1) = torch::zeros({1, tensor->size(1)});
    if (*lastNNZ > 0) *lastNNZ -= 1;
    return true;
  }

  bool TensorOP::deleteRowBufferMode(TensorPtr tensor, int64_t rowIdx, int64_t *lastNNZ) {
    return deleteRowBufferMode(tensor.get(), rowIdx, lastNNZ);
  }

  bool TensorOP::deleteRowsBufferMode(torch::Tensor *tensor, std::vector<int64_t> &rowIdx, int64_t *lastNNZ) {
    std::sort(rowIdx.begin(), rowIdx.end(), std::greater<int64_t>());
    for (int64_t value: rowIdx) {
      if (!deleteRowBufferMode(tensor, value, lastNNZ)) return false;
    }
    return true;
  }

  bool TensorOP::deleteRowsBufferMode(TensorPtr tensor, std::vector<int64_t> &rowIdx, int64_t *lastNNZ) {
  ...
  std::vector<uint8_t> TensorOP::tensorToFlatBin(torch::Tensor *tensor) {
    ...
    std::copy(reinterpret_cast<const uint8_t *>(&rows), reinterpret_cast<const uint8_t *>(&rows) + sizeof(int64_t),
              ruIter);
    ruIter += sizeof(int64_t);
    std::copy(reinterpret_cast<const uint8_t *>(&cols), reinterpret_cast<const uint8_t *>(&cols) + sizeof(int64_t),
              ruIter);
    ruIter += sizeof(int64_t);
    std::copy(reinterpret_cast<const uint8_t *>(tensor->data_ptr<float>()),
              reinterpret_cast<const uint8_t *>(tensor->data_ptr<float>() + tensor->numel()), ruIter);
    return ru;
  }
  ...
  bool TensorOP::tensorFromFlatBin(torch::Tensor *tensor, std::vector<uint8_t> &ru) {
    if (ru.size() < sizeof(int64_t) * 2) return false;
    int64_t rows, cols;
    std::copy(ru.begin(), ru.begin() + sizeof(int64_t), reinterpret_cast<uint8_t *>(&rows));
    std::copy(ru.begin() + sizeof(int64_t), ru.begin() + 2 * sizeof(int64_t), reinterpret_cast<uint8_t *>(&cols));
    uint64_t expectedSize = rows * cols * sizeof(float) + sizeof(int64_t) * 2;
    if (ru.size() < expectedSize) return false;
    *tensor = torch::from_blob(ru.data() + 2 * sizeof(int64_t), {rows, cols}, torch::kFloat32).clone();
    return true;
  }
  ...
  torch::Tensor TensorOP::rowSampling(torch::Tensor &tensor, int64_t sampledRows) {
    if (sampledRows >= tensor.size(0) || sampledRows <= 0) return tensor.clone();
    auto indices = torch::randperm(tensor.size(0), torch::kLong).slice(0, 0, sampledRows);
