...
VectorDBStream VectorDBStream::from(const std::vector<torch::Tensor> &input_tensors) {
...
VectorDBStream& VectorDBStream::map(const std::function<torch::Tensor(const torch::Tensor &)> &func) {
...
VectorDBStream& VectorDBStream::filter(const std::function<bool(const torch::Tensor &)> &predicate) {
...
VectorDBStream& VectorDBStream::map_to_embedding(const std::function<torch::Tensor(const std::string &)> &embedding_func) {
...
void VectorDBStream::to_sink(const std::shared_ptr<VectorDB> &vector_db) {
  for (const auto &tensor : tensors) {
    vector_db->insert_tensor(tensor);  // Insert each tensor into the vector database
  }
}

// Query: Perform real-time queries on the vector database using tensors
std::vector<torch::Tensor> VectorDBStream::query_nearest(const torch::Tensor &query_tensor, size_t k) {
  std::shared_ptr<VectorDB> vector_db = std::make_shared<VectorDB>(query_tensor.size(0), nullptr);
  return vector_db->query_nearest_tensors(query_tensor, k);  // Query nearest tensors in real-time
}