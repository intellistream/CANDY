...
bool VectorDB::insert_vector(const std::vector<float>& vec) {
  if (vec.size() != dimensions) {
    std::cerr << "Error: Vector dimensions do not match the expected size ("
              << dimensions << ")." << std::endl;
    return false;
  }

  {
    ...
    if (search_algorithm) {
      search_algorithm->insert(
          id, vec);  // Insert into the search algorithm's index
    }
  }
...
bool VectorDB::remove_vector(size_t id) {
  {
    ...
    if (search_algorithm) {
      search_algorithm->remove(
          id);  // Remove the vector from the search algorithm's index
    }
  }
...
std::vector<std::vector<float>> VectorDB::query_nearest_vectors(
    const std::vector<float>& query_vec, size_t k) const {
  if (query_vec.size() != dimensions) {
    std::cerr
        << "Error: Query vector dimensions do not match the expected size ("
        << dimensions << ")." << std::endl;
    return {};
  }
  ...
  if (!search_algorithm) {
    std::cerr << "Error: No search algorithm available for querying."
              << std::endl;
    return {};
  }
