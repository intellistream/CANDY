...
get_cuda_version() {
    ...
    if command -v /usr/local/cuda/bin/nvcc &> /dev/null; then
        ...
        echo "CUDA is not installed or nvcc is not in your PATH, try to install"
...
# Check if there is an NVIDIA GPU installed
if lspci | grep -i 'nvidia' > /dev/null; then
    echo "NVIDIA GPU is installed."
    get_cuda_version()
else
    echo "No NVIDIA GPU detected, go to the cpu-only set up."
    ./startWithoutCuda.sh
    exit 1
fi